...
export const MultiFieldEncoding = () => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto my-16 overflow-hidden flex justify-center bg-[var(--section-bg-dark)]/30 rounded-2xl border border-[var(--border-light)] p-8">
      <svg
        viewBox="0 0 900 480"
      ...
      >
        ...
        {/* Labels */}
        <text x="50" y="40" fill="var(--text-dim)" fontSize="12" letterSpacing="2" className="uppercase font-bold">Protobuf Fields</text>
        <text x="550" y="40" fill="var(--text-dim)" fontSize="12" letterSpacing="2" className="uppercase font-bold">Binary Stream (Hex)</text>
        ...
        {fields.map((field, i) => (
          <g key={field.id} transform={`translate(0, ${70 + i * 130})`}>
            ...
            >
              <rect x="50" y="10" width="280" height="80" rx="8" fill="var(--panel-bg)" stroke={field.color} strokeWidth="1" opacity="0.8" />
              <text x="70" y="40" fill={field.color} fontSize="14" className="font-bold">{field.label}</text>
              <text x="70" y="65" fill="var(--text-color)" fontSize="14">Value: {field.value}</text>
            </motion.g>
            ...
            <g transform="translate(530, 10)">
              {field.hex.map((h, j) => {
                ...
                for (let k = 0; k < j; k++) {
                   ...
                   currentX += (prevH.length > 2 ? (prevH.length * 10 + 20) : 60) + 10;
                }
                ...
                return (
                  <motion.g
                    ...
                    transition={{ duration: 0.4, delay: 0.8 + i * 0.2 + j * 0.1 }}
                  >
                    <rect 
                      x={currentX} 
                      y="15" 
                      ...
                      rx="4" 
                      fill={
                        field.labels[j] === 'Tag' ? 'var(--cyber-neon-blue)' :
                        field.labels[j] === 'Len' ? 'var(--cyber-neon-yellow)' :
