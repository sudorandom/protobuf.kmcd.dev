  ...
  return (
    <div className="space-y-8">
        ...
        <div className="p-4 bg-[var(--cyber-neon-blue)]/5 border border-[var(--cyber-neon-blue)]/20 rounded-lg flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-2 font-mono text-[var(--cyber-neon-blue)] text-lg md:text-xl">
            {(binaryString.match(/.{1,7}/g) || []).map((chunk, i) => (
              <span key={i} className="inline-block whitespace-nowrap bg-[var(--section-bg-dark)] px-1.5 py-0.5 rounded border border-[var(--cyber-neon-blue)]/20 shadow-inner">
                {chunk}
              </span>
            ))}
          </div>
        ...
        <div className="p-4 bg-[var(--cyber-neon-pink)]/5 border border-[var(--cyber-neon-pink)]/20 rounded-lg flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-2 font-mono text-lg md:text-xl">
            {varintBytes.map((byte, i) => {
              ...
              return (
                <span key={i} className="inline-block whitespace-nowrap bg-[var(--section-bg-dark)] rounded border border-[var(--cyber-neon-pink)]/20 shadow-inner overflow-hidden flex">
                  <span className={`px-1 py-0.5 ${binary[0] === '1' ? 'text-[var(--cyber-neon-pink)] bg-[var(--cyber-neon-pink)]/10' : 'text-[var(--text-dim)] bg-[var(--bg-color)]'}`}>
                    {binary[0]}
                  </span>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="p-6 bg-[var(--section-bg-dark)] border border-[var(--border-light)] rounded-lg relative">
          {/* Header Row */}
          ...
          <div className="flex flex-col gap-6 max-h-[600px] overflow-y-auto pr-4 custom-scrollbar">
            {bitGroups.map((group, i) => {
              ...
              return (
                <div key={i} className="flex flex-col sm:grid sm:grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-4 group/bitgroup border-b border-[var(--border-light)] pb-6 sm:pb-0 sm:border-0 last:border-0">
                  ...
                  <div className="flex flex-col items-center sm:items-end gap-1 w-full sm:w-auto">
                    <div className="flex items-center gap-3">
                      <div className="p-2 border border-dashed border-[var(--border-light)] rounded font-mono text-lg text-[var(--text-dim)] group-hover/bitgroup:border-[var(--cyber-neon-cyan)]/50 group-hover/bitgroup:text-[var(--cyber-neon-cyan)] transition-colors">
                        {group}
                      </div>
                    </div>
                    <span className="hidden sm:inline text-[9px] font-mono text-[var(--text-dim)] uppercase">Group {i}</span>
                  </div>

                  {/* Middle: Connector */}
                  <div className="flex flex-col sm:flex-row items-center gap-1">
                    <div className="h-4 w-px sm:h-px sm:w-16 bg-gradient-to-b sm:bg-gradient-to-r from-[var(--cyber-neon-blue)]/40 to-[var(--cyber-neon-pink)]/40 relative">
                      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 -mb-[1px] w-1.5 h-1.5 border-b border-r border-[var(--cyber-neon-pink)]/40 rotate-45 sm:bottom-auto sm:top-1/2 sm:left-full sm:-translate-y-1/2 sm:-ml-[1px] sm:rotate-[-45deg] sm:border-b-0 sm:border-t"></div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center sm:items-start gap-1 w-full sm:w-auto">
                    <div className="flex items-center gap-3">
                      <div className="flex border border-[var(--cyber-neon-pink)]/30 rounded overflow-hidden shadow-[0_0_10px_rgba(255,0,255,0.05)] group-hover/bitgroup:shadow-[0_0_15px_rgba(255,0,255,0.15)] transition-all">
                        <div className={`px-1.5 py-1.5 font-mono text-lg leading-none ${targetBinary[0] === '1' ? 'bg-[var(--cyber-neon-pink)]/20 text-[var(--cyber-neon-pink)]' : 'bg-[var(--bg-color)] text-[var(--text-dim)]'}`}>
                          {targetBinary[0]}
                        </div>
                        <div className="px-1.5 py-1.5 bg-[var(--cyber-neon-cyan)]/10 text-[var(--cyber-neon-cyan)] font-mono text-lg leading-none">
                          {targetBinary.slice(1)}
                        </div>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[8px] font-mono text-[var(--text-dim)] uppercase">Byte {i}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8 pt-6 border-t border-[var(--border-light)]">
            <div className="flex items-center gap-3">
