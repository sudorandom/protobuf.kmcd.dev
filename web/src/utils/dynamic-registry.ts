 ...
 */
export async function createDynamicRegistry(
  protoSource: string,
): Promise<RegistryResult> {
  console.log(
    "createDynamicRegistry: Parsing proto source...",
    protoSource.substring(0, 100) + "...",
  );
  ...
  if ("compilationErrors" in result && result.compilationErrors) {
    console.log(
      "createDynamicRegistry: Compilation errors found:",
  ...
  console.log(
    "createDynamicRegistry: Successfully parsed to FileDescriptorSet",
  );
  ...
  const allFiles = [...standardFiles, ...fds.file];
  ...
  if (!file) {
    ...
    throw new Error(
      "Could not find any user-defined messages in the provided proto source.",
    );
  }
