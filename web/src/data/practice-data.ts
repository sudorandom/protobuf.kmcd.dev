...
export const EXERCISES: ExerciseDef[] = [
  {
    ...
    scenario: "Protocol Buffers use unique numeric tags (field numbers) to identify fields in their compiled binary format instead of sending long string names over the wire. This makes the payload incredibly compact and fast to parse. However, if a field is not assigned a number, the schema cannot compile.\n\nFor example, a valid message with a single field looks like this:\n\n```protobuf\nmessage Example {\n  string some_field = 1;\n}\n```\n\nHere, we are defining a UserProfile message, but we haven't assigned the mandatory numbers to ID, name, and email fields.",
...
`,
    assertions: [
      ...
      },
      {
        ...
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "UserProfile");
          if (!msg) throw new Error("Message 'UserProfile' not found.");
        }
      },
      {
        id: "fields_have_numbers",
        description: "All fields (id, name, email) are assigned unique numbers.",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file.messageType.find((m: any) => m.name === "UserProfile");
          ...
          if (!idField || !idField.number) throw new Error("Field 'id' needs a valid field number.");
          if (!nameField || !nameField.number) throw new Error("Field 'name' needs a valid field number.");
          if (!emailField || !emailField.number) throw new Error("Field 'email' needs a valid field number.");
  ...
  },
  {
    ...
    scenario: "Consistency is critical when generating SDKs across different programming languages. The official Protobuf style guide dictates that message names should use PascalCase (e.g. UserProfile) and field names should use snake_case (e.g. user_id).\n\nIf you use camelCase inside a schema, plugins converting schemas to JSON or stub stencils will produce inconsistent or broken interfaces in target languages (like Go, Java, or TypeScript).",
    ...
    assertions: [
      {
        ...
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const goodMsg = file?.messageType.find((m: any) => m.name === "UserProfile");
          if (!goodMsg) throw new Error("Message 'UserProfile' not found.");
        }
      },
      {
        id: "pascal_case_msg",
        description: "The old snake_case message name 'user_profile' is removed.",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const badMsg = file?.messageType.find((m: any) => m.name === "user_profile");
          if (badMsg) throw new Error("Message name 'user_profile' should be PascalCase: 'UserProfile'.");
        }
      },
      {
        id: "snake_case_fields",
        description: "Field names use snake_case (user_id, email_address, profile_image).",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "UserProfile");
          if (!msg) throw new Error("Message 'UserProfile' not found.");
          
          if (msg.field.some((f: any) => f.name === "userId")) throw new Error("Field 'userId' must be snake_case: 'user_id'.");
          if (msg.field.some((f: any) => f.name === "emailAddress")) throw new Error("Field 'emailAddress' must be snake_case: 'email_address'.");
          if (msg.field.some((f: any) => f.name === "profileImage")) throw new Error("Field 'profileImage' must be snake_case: 'profile_image'.");
          
          const hasUserId = msg.field.some((f: any) => f.name === "user_id");
          const hasEmail = msg.field.some((f: any) => f.name === "email_address");
          const hasImage = msg.field.some((f: any) => f.name === "profile_image");
...
`,
    assertions: [
      {
        id: "ledger_type",
        description: "ledger_balance uses 'sint64' for zigzag encoding efficiency.",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "Metrics");
          const ledger = msg?.field.find((f: any) => f.name === "ledger_balance");
          if (!ledger) throw new Error("Field 'ledger_balance' not found.");
          if (ledger.type !== 18) throw new Error("ledger_balance type must be 'sint64' (found type " + ledger.type + ").");
        }
      },
      {
        id: "counter_type",
        description: "hardware_counter uses unsigned integer type ('uint32' or 'uint64').",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "Metrics");
          const counter = msg?.field.find((f: any) => f.name === "hardware_counter");
          if (!counter) throw new Error("Field 'hardware_counter' not found.");
          if (counter.type !== 13 && counter.type !== 4) {
            throw new Error("hardware_counter must be 'uint32' (13) or 'uint64' (4). Found: " + counter.type);
          }
        }
      }
    ]
  },
  {
    ...
    scenario: "To represent lists, arrays, or sequences of elements in Protobuf, we prepend the field declaration with the 'repeated' keyword. In proto3, repeated fields of basic numeric and boolean types use packed encoding by default, packing multiple values into a single contiguous block of bytes on the wire for maximum compression.",
    ...
    assertions: [
      ...
      },
      {
        ...
        validate: (fds: any) => {
          ...
          if (tags?.label !== 3) { // LABEL_REPEATED = 3
...
`,
    assertions: [
      {
        ...
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "UserAccount");
          if (!msg) throw new Error("UserAccount message not found.");
          
          const hasReserved3 = msg.reservedRange.some((r: any) => r.start <= 3 && r.end > 3);
          if (!hasReserved3) throw new Error("Field number 3 must be reserved. Add 'reserved 3;'");
          
          if (msg.field.some((f: any) => f.number === 3)) {
            throw new Error("A field is still using tag number 3! Remove it before reserving.");
          }
        }
      },
      {
        ...
        validate: (fds: any) => {
          ...
          if (!hasReservedPhone) throw new Error("Field name 'phone' must be reserved. Add 'reserved \"phone\";'");
          
          if (msg.field.some((f: any) => f.name === "phone")) {
            throw new Error("Field name 'phone' is still defined in the message! Remove it before reserving.");
          }
        }
      }
    ]
  },
  {
    ...
    scenario: "In proto3, basic types like boolean are omitted from wire serialization if they have their default value (false). This makes it impossible to distinguish between an unset field and a false value. We need explicit presence for the 'is_admin' field.",
...
`,
    assertions: [
      {
        id: "optional_admin",
        description: "Field 'is_admin' uses the 'optional' keyword for presence.",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "UserSession");
          ...
          if (!isAdmin.proto3Optional) {
            throw new Error("Field 'is_admin' must have explicit presence. Prepend the 'optional' keyword.");
          }
        }
      }
    ]
  },
  {
    ...
    scenario: "A notification targets either a specific user_id or a generic email address, but must never contain both at the same time.",
...
`,
    assertions: [
      {
        ...
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "NotificationTarget");
          if (!msg) throw new Error("NotificationTarget message not found.");
          const oneof = msg.oneofDecl.find((o: any) => o.name === "identifier");
          if (!oneof) throw new Error("No oneof declaration named 'identifier' was found.");
        }
      },
      {
        id: "fields_in_oneof",
        description: "Both 'user_id' and 'email' fields are members of the 'identifier' oneof.",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          const msg = file?.messageType.find((m: any) => m.name === "NotificationTarget");
          const oneofIdx = msg.oneofDecl.findIndex((o: any) => o.name === "identifier");
          ...
          if (userId.oneofIndex !== oneofIdx || userId.proto3Optional) {
            throw new Error("Field 'user_id' is not inside the 'identifier' oneof.");
          }
          if (email.oneofIndex !== oneofIdx || email.proto3Optional) {
            throw new Error("Field 'email' is not inside the 'identifier' oneof.");
          }
        }
      }
    ]
  },
  {
    ...
    scenario: "We need to track when an event occurred. Storing timestamps as custom integers or strings is error-prone. Standardizing on Protobuf's official Timestamp type is the best practice.",
    ...
    assertions: [
      {
        ...
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          if (!file) throw new Error("Package 'practice' not declared.");
          const hasImport = file.dependency.includes("google/protobuf/timestamp.proto");
          if (!hasImport) throw new Error("Missing import of 'google/protobuf/timestamp.proto'.");
        }
      },
      {
        id: "timestamp_field",
        description: "Field 'created_at' of type 'google.protobuf.Timestamp' exists.",
        validate: (fds: any) => {
          const file = fds.file.find((f: any) => f.package === "practice");
          ...
          if (createdAt.type !== 11) throw new Error("Field 'created_at' must be a message type.");
          if (createdAt.typeName !== ".google.protobuf.Timestamp") {
            throw new Error("Field 'created_at' must be of type 'google.protobuf.Timestamp'. Found: " + createdAt.typeName);
          }
