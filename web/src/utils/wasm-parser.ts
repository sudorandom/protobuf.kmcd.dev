...
export async function initWasm() {
  ...
  wasmPromise = (async () => {
    if (typeof window === "undefined") {
      ...
      const { TextEncoder, TextDecoder } = await import(
        /* @vite-ignore */ "node:util"
      );
    ...
    // We check if the response is gzipped by looking at the magic number (0x1f 0x8b)
    // This allows us to support both compressed and uncompressed files (useful for dev).
...
export async function convertToPrototext(
  messageName: string,
  fileDescriptorSet: Uint8Array,
  jsonString: string,
): Promise<string> {
...
export async function convertToProtoscope(
