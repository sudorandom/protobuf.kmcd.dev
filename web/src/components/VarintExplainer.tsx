    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-[var(--cyber-neon-blue)]/5 border border-[var(--cyber-neon-blue)]/20 rounded-lg flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono text-[var(--cyber-neon-blue)] uppercase tracking-widest">Standard Binary</span>
              <span className="text-xs font-mono text-[var(--cyber-neon-blue)]/90 bg-[var(--cyber-neon-blue)]/10 px-2 py-0.5 rounded">{standardBinary.length} BITS</span>
            </div>
          ...
          <div className="p-4 bg-[var(--cyber-neon-cyan)]/5 border border-[var(--cyber-neon-cyan)]/20 rounded-lg flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono text-[var(--cyber-neon-cyan)] uppercase tracking-widest">7-Bit Partitioning</span>
              <span className="text-xs font-mono text-[var(--cyber-neon-cyan)]/90 bg-[var(--cyber-neon-cyan)]/10 px-2 py-0.5 rounded">{partitionedBinary.length} BITS</span>
            </div>
            <div className="flex flex-wrap gap-x-3 gap-y-2 font-mono text-[var(--cyber-neon-cyan)] text-base">
              {(partitionedBinary.match(/.{1,7}/g) || []).map((chunk, i) => (
                <span key={i} className="inline-block whitespace-nowrap bg-[var(--section-bg-dark)] px-1.5 py-0.5 rounded border border-[var(--cyber-neon-cyan)]/20 shadow-inner">
                  {chunk}
                </span>
              ))}
            </div>
        ...
        <div className="p-4 bg-[var(--cyber-neon-pink)]/5 border border-[var(--cyber-neon-pink)]/20 rounded-lg flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono text-[var(--cyber-neon-pink)] uppercase tracking-widest">Final Varint (Little-Endian)</span>
            ...
            {varintBytes.map((byte, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-[10px] font-mono text-[var(--cyber-neon-pink)] uppercase">0x{byte.toString(16).padStart(2, '0').toUpperCase()}</span>
                </div>
              );
            })}
          </div>
